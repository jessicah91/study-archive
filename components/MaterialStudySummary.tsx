...
export default function MaterialStudySummary({
  ...
  const [source, setSource] = useState<"database" | "openai" | null>(null);
  ...
  async function loadSavedSummary() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/materials/summarize?materialId=${encodeURIComponent(materialId)}`,
        {
          method: "GET",
          cache: "no-store",
        }
      ...
      if (!contentType?.includes("application/json")) {
        throw new Error(
          `저장된 요약을 확인하지 못했습니다. 상태 코드: ${response.status}`
        );
      }
      ...
      if (!response.ok) {
        throw new Error(
          result.detail
            ? `${result.error ?? "요약 조회 실패"}: ${result.detail}`
            : result.error ?? "저장된 요약 조회에 실패했습니다."
        );
      }
  ...
  async function generateSummary(forceRegenerate = false) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.detail
            ? `${result.error ?? "요약 생성 실패"}: ${result.detail}`
            : result.error ?? "AI 요약 생성에 실패했습니다."
        );
      }

      if (!result.summary) {
        throw new Error("서버 응답에 요약 결과가 없습니다.");
      }
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6">
      ...
      >
        {isLoading ? "AI가 요약 중이에요..." : "AI 요약 만들기"}
      </button>
    </section>
  );
}
  ...
  >
    {isLoading ? "다시 생성 중..." : "AI 요약 다시 만들기"}
  </button>
</div>
      </div>
      ...
      <div>
        <h3 className="mb-2 font-bold text-slate-900">전체 요약</h3>
      ...
      <div>
        ...
        {summary.examPoints.length > 0 ? (
          <div className="space-y-3">
            {summary.examPoints.map((point, index) => (
              <div
                key={`${point}-${index}`}
