  ...
  const loadContentStatus =
    useCallback(async () => {
      ...
      if (error) {
        ...
        setMessage(
          `추출 상태를 확인하지 못했어요: ${error.message}`,
        );
      } else {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-400">
            {isExtracted
              ? `추출 완료 · ${
                  content?.extracted_text?.length ?? 0
                }자`
...
export default function WeekMaterials({
  ...
  const loadMaterials = useCallback(async () => {
    ...
    if (error) {
      ...
      setMessage(
        `자료 목록을 불러오지 못했어요: ${error.message}`,
      );
  ...
  async function uploadFile(file: File) {
    ...
    if (uploadError) {
      ...
      setMessage(
        `파일을 업로드하지 못했어요: ${uploadError.message}`,
      );
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {materials.map((material) => (
              <article
                key={material.id}
                className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-start"
              >
                ...
                <div className="flex shrink-0 gap-2">
                  <button
                    ...
                    className="flex-1 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700 sm:flex-none"
                  >
                    열기
                  </button>
                  ...
                  <button
                    ...
                    className="flex-1 rounded-xl border border-red-100 px-4 py-2 text-sm font-semibold text-red-500 transition hover:bg-red-50 sm:flex-none"
                  >
                    삭제
                  </button>
                </div>
